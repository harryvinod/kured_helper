...
class LeaderElection:
    ...
    def run_leader_election(self):
        ...
        while True:
            try:
                ...
                # my_env_var_value = os.environ.get("pd-service-name")
                # logging.info(f"PD Service name is: {my_env_var_value}")
    ...
    def try_acquire_leader(self):
        ...
        try:
            ...
            # Check if kured has selected a node to reboot. We skip the election if there's no node seleted.
            if not node_with_kured_annotation:
                logging.info("No nodes with Kured annotation. Skipping leader election.")
                return False, node_with_kured_annotation, time_created
            
            # Check if the current node is selected by kured for reboot. We skip the election on the pod if its about to reboot.
            if current_node_id in node_with_kured_annotation:
                logging.info("Current node already has Kured annotation. Skipping leader election.")
                return False, node_with_kured_annotation, time_created
            
            # If a leader is elected.
            if current_lead:
                ...
                if current_lead not in nodes_set:
    ...
    def perform_actions_as_leader(self, node_with_kured_annotation, time_created):
        ...
        node_with_annotation, current_leader, timestamp = self.get_node_with_kured_annotation()
        current_node_id = os.environ.get("NODE_ID")
        if not current_leader == current_node_id:
            success = False
            return success, timestamp
        logging.info("Leader validated successfully. Performing actions as leader...")
        success = False
        try:
            ...
            if response and response.get("success"):
                logging.info("Checkout service responded successfully. Waiting for kured to reboot the node")
                # Keep checking if the node has been restarted
        ...
        while True:
            ...
            # Get the current node object
                ...
                if not node.spec.unschedulable:
                    logging.info(f"Node {node_id} is already uncordoned.")
                    return
